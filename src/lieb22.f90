...
SUBROUTINE TMMultLieb2DAtoB1(PSI_A,PSI_B, Ilayer, En, DiagDis, M )
  ...
  REAL(KIND=CKIND) new , PsiRight, PsiLeft, stub
  ...
  DO xSiteS=1,LiebSpacer*M   
     ...
     CASE(10)
        IF(Mod(xSiteS,LiebSpacer)==1) THEN
           OnsitePotVec(xSiteS,jSite)= -En + DiagDis*(DRANDOM(ISeedDummy)-0.5D0)
        ELSE
           OnsitePotVec(xSiteS,jSite)= -En + SmallDisOnRim*(DRANDOM(ISeedDummy)-0.5D0)
        END IF
     CASE(20)
        IF(Mod(xSiteS,LiebSpacer)==1) THEN
           OnsitePotVec(xSiteS,jSite)= -En + DiagDis*(DRANDOM(ISeedDummy)-0.5D0)*SQRT(12.0D0)
        ELSE
           OnsitePotVec(xSiteS,jSite)= -En + SmallDisOnRim*(DRANDOM(ISeedDummy)-0.5D0)*SQRT(12.0D0)
        END IF
      CASE(30)
        IF(Mod(xSiteS,LiebSpacer)==1) THEN
           OnsitePotVec(xSiteS,jSite)= -En + GRANDOM(ISeedDummy,0.0D0,DiagDis)
        ELSE
           OnsitePotVec(xSiteS,jSite)= -En + GRANDOM(ISeedDummy,0.0D0,SmallDisOnRim)
        END IF
     END SELECT
  END DO

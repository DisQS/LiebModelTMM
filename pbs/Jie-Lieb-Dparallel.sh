...
boundary=${2:-1}
liebdef=${3:-31}
flux0=${4:-1.0}  # This is a energy loop with fix disorder(in flux loop below) 
flux1=${5:-2.0}
dflux=${6:-0.1}
...
for flux in 16.6 16.7 #16.8 16.9 17.1 17.2 17.3 17.4 17.5 17.6 17.7 17.8 # this is disorder value 
do
    
jobname="L$liebdef-M$width-D$flux"
echo $jobname

jobfile=`printf "$jobname.sh"`
logfile=`printf "$jobname.log"`

inpfile=tmseLMxD-D$flux.inp
...
echo "CubeConPot    = -10.0     ">> $inpfile
echo "DiagDis0      = $flux     ">> $inpfile
echo "DiagDis1      = $flux     ">> $inpfile
echo "dDiagDis      = $dflux    ">> $inpfile
echo "LiebConPot    = -5.0      ">> $inpfile
echo "LiebDis       = 0.0       ">> $inpfile
echo "Energy0       = $flux0    ">> $inpfile
echo "Energy1       = $flux1    ">> $inpfile
echo "dEnergy       = $dflux    ">> $inpfile
echo "Kappa         = 1.0       ">> $inpfile
echo "Epsilon       = 1.0E-3    ">> $inpfile
